...
single-message readout constancy.  Bare satisfiability plus a readout map is not enough: the
two-point Boolean model already has two valid SAT-search outputs with different readouts, so no
message can be correct for all valid outputs.  Separately,
...
the required quarter-mass tower bound is false.  The product-bound contrapositive now reaches the
...
first success event, the one-cell field's only accumulated-history cell contains no failed point
for the second repeated success test.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  witness it is equivalent to single-message constancy.
  ...
  product-bound contrapositive.  The fixed-field version is now blocked at the same level: when
  ...
  localizes any such failure to a concrete fielded position, and in the one-cell repeated model the
  second conditioned cut is explicitly the failed same-cell matching/certificate obligation.
\end{enumerate}
