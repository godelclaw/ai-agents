...
\texttt{ActualSwitchedLocalBitFallbackZeroSampleImage.lean} & structural zero-sample image equivalence: the radius-zero wrapper has exactly the fallback decoder predictor image, so finite-image budgets and clocked exact-visible realizations are equivalent on both sides \\
...
refutes the manuscript's exact-support premise.  So the post-switch
randomized-residual repair no longer survives by retreating from positive
doubled advantage to the strict half-accuracy formulation.
...
and on the exact post-switch support surface it remains directly blocked.
...
records three separate narrow covered entries for this cluster: the sample-level
majority visible-budget boundary, the bit-fallback visible-budget boundary, and
the fallback-side \(q\)-threshold
\texttt{PNPKpolySubrepairClass.actualLocalBoundedSampleMajorityBitFallbackRecoveryLightestPointBoundary},
with route anchors for both the generic surjective fallback decoder and the
canonical \texttt{exactVisibleRuleDecode} specialization.  The latest update
also adds the joint exact-decoder theorem above, so the covered \texttt{Kpoly}
subledger now has a single synthesis-level endpoint where fallback-side
visible-budget failure and fallback-side lightest-point recovery failure are
...
the alphabet-base factor can be rounded down for free.
...
Lean now packages the stronger structural form in
...
the successor bit on \(|\texttt{surface}|+1\) cannot be rounded away.
...
far from a free truth-table oracle.
...
and projected bit-vector ERM specializations expose the same objects.  This is
a positive route for the actual canonical family, not a product-success
shortcut: the manuscript must still prove that its switched predictors are the
family realized by such shared-extractor codes or by the named ERM wrapper.
...
measure.  The broad
\texttt{kpolyCompressionBridge} remains uncovered until the actual switched
predictor family is proved small.
...
finite-image and quotient-code pullback obstructions, while keeping the broad
\texttt{kpolyCompressionBridge} class on the uncovered side.
...
\texttt{PNPKpolySubrepairClass} items.  The broad
\texttt{kpolyCompressionBridge} remains uncovered because these theorems only
