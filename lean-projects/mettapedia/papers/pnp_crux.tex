...
has the same message readout.  Bare satisfiability plus a readout map is not enough: the two-point
Boolean model already has two satisfying witnesses with different readouts.  Separately,
...
proof can exist at all.  The v13 layer specializes this to abstract atom ledgers, semantic
concrete-cell certificates, and ordered fixed-field certificates.  Thus a proposed switched list
must first pass the tower-product count check before any finer cell structure can rescue it.  The
v13 instantiation layer now sharpens this further.
...
\texttt{V13HistoryFieldDeterminesSuccess} says that every field cell is homogeneous for the next
success event, and
\texttt{not\_v13FieldPrefixInstantiation\_of\_fieldDeterminesSuccess\_positive\_success} proves
that any such field is certifiable only when the next-success prefix has zero mass.  Thus a proposed
v13 history field cannot already decide the next success bit on any positive-mass prefix; it must
leave enough same-cell counterexamples to satisfy the success-versus-failure balance.  The Boolean
first-coordinate field now fails by this general theorem, not only by an arithmetic calculation on
one named cell.  The blocker has also been lifted from the head of a fielded suffix to an arbitrary
...
proof exists, then the supplied field at that position cannot determine the next success bit.
The same v13-specific module also checks the smallest
positive-mass failure mode for sequential repetition: if the second switched step repeats the first
exact-success event, then the first success prefix has count \(2\), the repeated-success prefix
still has count \(2\), and the second prefix half-step is false.  Thus no v13 switching certificate
can exist for this repeated two-step switching claim.  The same example also fails the global
product-bound audit: two identical half-mass success events still have half-mass intersection, so
the required quarter-mass tower bound is false.
...
The current Lean answer to all twenty-three is ``yes.''  Therefore the published proof cannot be
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  admissibility before one inspects any individual prefix.
  ...
  premise is strictly stronger than global half-success:
  ...
  supplied field that determines the next success bit cellwise: on a positive-mass success prefix,
  such a field blocks the whole fielded suffix, and the theorem applies at any later fielded
  position after the preceding success-event history has been accumulated.  In the operational
  matching formulation, any recursive matching proof on such a positive-mass prefix directly
  proves that the supplied field is not success-determining.
  \item A repeated exact-success event gives a checked positive-mass obstruction to that atom-ledger
  obligation: the second repeated step has conditional success \(1\), so no v13 switching
  instantiation exists for the two-step repeated Boolean-square model.  It also violates the
  global tower-product inequality, blocking both abstract and concrete v13 certificates by the new
  product-bound contrapositive.
\end{enumerate}
