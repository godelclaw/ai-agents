...
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeFixedPressureExactObstruction.lean}} & fixed-pressure exact witness/global-output obstruction for the one-one two-mode branch \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeFixedPressureExactClassification.lean}} & fixed-pressure exact witness/global-output classification for the one-one two-mode branch \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeSchwartzBKMBridge.lean}} & bounded two-mode Schwartz witnesses supply their own BKM premise once the momentum equation closes \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeFixedPressureBKMObstruction.lean}} & fixed-pressure BKM packaging does not relax the pressure-gradient residual obstruction \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeFixedPressureBKMClassification.lean}} & fixed-pressure BKM data exist exactly at the pressure-gradient residual matching boundary \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeFixedPressureUniformObstruction.lean}} & uniform-vorticity packaging preserves the same fixed-pressure zero-pressure obstruction \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeFixedPressureUniformClassification.lean}} & uniform-vorticity data exist exactly at the same fixed-pressure residual matching boundary \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeBKMClause.lean}} & exact bounded two-mode momentum closure inhabits both BKM continuation clause surfaces \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesFiniteModeUniformClause.lean}} & exact bounded two-mode momentum closure inhabits both uniform-vorticity continuation clause surfaces \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesPressureGaugeRigidity.lean}} & pressure repairs at fixed velocity are rigid modulo zero spatial-gradient gauges \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesSpatialAffinePressureGaugeRigidity.lean}} & zero-localized spatial-affine pressure repairs collapse to equality of the affine coefficient \\
...
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesConstantFieldWitnessObstruction.lean}} & nonzero whole-space constant fields satisfy the non-energy witness fields but fail bounded energy \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesComponentShadowIncoherence.lean}} & scalar component shadows can use split existential witnesses, so they do not recover the concrete vector clause \\
\shortstack[l]{\texttt{Mettapedia/FluidDynamics/}\\\texttt{NavierStokes/NavierStokesSharedWitnessBridgeObstruction.lean}} & no uniform bridge upgrades the current scalar component shadow to a coherent shared-witness component clause \\
...
Lean now also records a separate incoherence in the current scalar
component-shadow surface.  In
\texttt{NavierStokesComponentShadowIncoherence.lean},
\texttt{componentShadowZeroOutput} shows that if one coordinate of the initial
velocity vanishes, the scalar component clause can be inhabited by the zero
scalar field even though the initial-condition slot is witnessed by a different
time-independent vector field.  The theorem
\texttt{componentFeffermanClause\_without\_concreteClause\_constantInitialVelocity}
specializes this to nonzero constant initial data with one zero coordinate:
the scalar component clause holds, while the full concrete vector-valued
Navier--Stokes clause is false by the whole-space bounded-energy obstruction
for nonzero constants.  Consequently
\texttt{not\_componentFeffermanClause\_implies\_concreteNavierStokesGlobalRegularityClause}
rules out a universal bridge from the present scalar component shadow back to
the concrete vector theorem surface.  Any repaired component route must carry a
shared vector witness or an equivalent coherence condition; the current
per-slot existential shadow is too weak.

The shared-witness repair has also been formalized and immediately classified.
\texttt{NavierStokesSharedWitnessComponent.lean} defines
\texttt{SharedWitnessComponentRegularityClause}, a scalar component output
backed by one concrete vector-valued
\texttt{NavierStokesGlobalRegularityWitness}.  Lean proves
\texttt{sharedWitnessComponentRegularityClause\_iff\_globalClause}: this repair
is exactly equivalent to the full concrete vector theorem, not a weaker scalar
surrogate.  Then
\texttt{NavierStokesSharedWitnessBridgeObstruction.lean} proves
\texttt{not\_uniform\_componentShadowBridge\_to\_sharedWitnessComponent} and
the three coordinate-specific variants.  Thus the present scalar component
shadow cannot be uniformly upgraded into even this minimal coherent repair; any
componentwise rescue has to change the predicate surface itself, not merely add
a bridge theorem downstream.
...
The nonzero constant-field route has the opposite failure mode.  The salvaged
module \texttt{NavierStokesConstantFieldWitnessObstruction.lean} proves
\texttt{constantVelocityField\_zeroSpatialGradientPressure\_nonEnergyFiniteTimeFields}:
with any smooth zero-spatial-gradient pressure gauge, a constant velocity field
has the smoothness, momentum, incompressibility, and initial-match fields of an
exact finite-time witness.  Lean then proves
\texttt{not\_exists\_constantVelocityField\_finiteTimeWitness\_of\_nonzero} and
the time-only-gauge package
\texttt{constantVelocityField\_timeOnlyPressure\_exhibits\_nonEnergyFiniteTimeFields\_without\_exactWitness}.
Thus this ansatz fails at the bounded-energy slot alone on \(\mathbb{R}^3\),
not because of a hidden pressure-gauge mismatch or a missing smoothness proof.
...
records that the seed still carries every other whole-space output field.  The
existential no-go
\texttt{not\_exists\_ExplicitConcreteNavierStokesGlobalOutputWithVelocityPressure\_boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_of\_stationaryMomentum\_failure}
now closes the whole smooth zero-gradient pressure repair class at the exact
global-output layer from the same single failed zero-pressure residual.  The
fixed-gauge classification
\texttt{boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_globalOutputWithVelocityPressure\_iff\_stationaryMomentum\_zeroPressure}
also makes the positive boundary exact: for each chosen smooth pressure with
zero spatial gradient, the fixed velocity/pressure global-output proposition
holds exactly when the zero-pressure stationary residual vanishes everywhere.
The fixed-gauge candidate theorem
\texttt{boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_exhibits\_nonMomentumGlobalFields\_without\_exactGlobalOutput}
also records the corresponding separation for each individual smooth
zero-gradient pressure: the velocity, pressure regularity, incompressibility,
initial match, and global energy fields are all present, but the exact
global-output predicate is still impossible under the same residual failure.
...
to satisfying that same zero-pressure stationary audit.  The fixed-gauge
variants
\texttt{boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_uniformVorticityData\_iff\_zeroPressure}
and
\texttt{boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_BKMData\_iff\_zeroPressure}
make the quotient statement pointwise: each individual smooth zero-gradient
pressure gauge has exactly the same packaged-data proposition as the literal
zero-pressure representative.  Finally, the direct no-go forms
\texttt{not\_exists\_boxedPartialPeriodizationSteadySeed\_anyZeroSpatialGradientPressure\_uniformVorticityData\_of\_stationaryMomentum\_failure}
and
\texttt{not\_exists\_boxedPartialPeriodizationSteadySeed\_anyZeroSpatialGradientPressure\_BKMData\_of\_stationaryMomentum\_failure}
turn one failed zero-pressure stationary residual into a single certificate
ruling out the entire smooth zero-gradient pressure repair class at both
packaged-data layers.
...
already subsumed.  The fixed-gauge classification
\texttt{boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_ExactPairWitnessConstructionRoute\_iff\_stationaryMomentum\_zeroPressure}
now states the same boundary for each chosen smooth zero-spatial-gradient
pressure field, matching the BKM-data and uniform-vorticity-data surfaces.
More strongly,
...
zero-pressure stationary residual fails at one slab point.  The fixed
zero-gradient candidate theorem
\texttt{boxedPartialPeriodizationSteadySeed\_zeroSpatialGradientPressure\_exhibits\_nonMomentumFiniteTimeAndGlobalFields\_without\_exactPairWitnessConstructionRoute}
also records the matching separation for each individual smooth zero-gradient
pressure field: finite-time smoothness, pressure regularity, incompressibility,
initial match, slab energy, and global bounded energy are all present, but the
whole bundled route is still impossible under the same residual failure.  The
older fixed time-only failure corollary
...
every time horizon.  On nonnegative slabs,
\texttt{timeIndependentVelocity\_has\_constantBKMEnvelope} exposes the actual
constant envelope and its linear integral bound for the same single-profile
seed.  The witness-construction audit now consumes the uniform
...
amplitude bounds.  The constant-one branch used by the finite-mode obstruction
route is now packaged explicitly in
\texttt{uniformVorticityBoundUpTo\_oneOneTwoModeSchwartzVelocity} and
\texttt{oneOneTwoModeSchwartzVelocity\_exhibits\_BKMEnvelope}, with bound equal
to the sum of the two profile bounds.  On nonnegative slabs, the companion
constant-envelope theorems
\texttt{twoModeSchwartzVelocity\_has\_constantBKMEnvelope\_of\_abs\_le} and
\texttt{oneOneTwoModeSchwartzVelocity\_has\_constantBKMEnvelope} expose the
actual envelope \(\Omega(t)=B\) and the linear integral bound \(T B\), rather
than only an existential BKM package.  The external-budget variants
\texttt{timeIndependentVelocity\_has\_constantBKMEnvelope\_of\_bound\_ge} and
\texttt{twoModeSchwartzVelocity\_has\_constantBKMEnvelope\_of\_abs\_le\_of\_bound\_ge}
also allow downstream arguments to replace the internally computed seminorm
budget by any larger constant budget without reopening the vorticity estimate.
Consequently,
...
output is already impossible.  Lean therefore now names the repaired boundary
directly as
\texttt{ExplicitFiniteEnergyBKMContinuationTargetOnNonnegHorizons}; the theorems
\texttt{ExplicitFiniteEnergyBKMContinuationTargetOnNonnegHorizons\_implies\_BKMContinuationClause\_allNonnegHorizons}
and
\texttt{ExplicitFiniteEnergyBKMContinuationTargetOnNonnegHorizons\_implies\_uniformVorticityContinuationClause\_allNonnegHorizons}
record exactly what can still be exported to the unrepaired BKM and
uniform-vorticity continuation clauses on nonnegative horizons.  So only these
repaired finite-energy, nonnegative-horizon formulations remain analytically
meaningful.
